...
export default new Vuex.Store({
  state: {
    ...
    unread: null, // 未读站内信
    unreadAndReply:null,//未读的已回复反馈
    showTopApp: true, //顶部APP下载显示
    ...
    userData: {
      ...
      avator: '/static/images/level/default.png', // 等级图标，手动配置
  ...
  getters: {
    ...
    showTopApp(state, getters) {
      return state.showTopApp;
    },
    ...
    unreadAndReply(state, getters) {
      if (state.unreadAndReply == null) {
        // 异步的一种获取
        countUnreadSggestReplyInfo().then(res => {
          if (res.success) {
            state.unreadAndReply = res.data.unreadCount ? res.data.unreadCount : 0
          }
        })
      }
      return state.unreadAndReply
    },
  ...
  mutations: {
    ...
    [types.SHOW_TOPAPP](state, val) {
      state.showTopApp = val
    },
    // 设置未读站内信
    ...
    // 读取一条未读的已回复反馈
    [types.READ_ANDREPLY](state) {
      if (state.unreadAndReply > 0) {
        state.unreadAndReply--
      } else {
        state.unreadAndReply = 0
      }
    },
    // 设置底部
    ...
    [types.UPDATE_MONEY]({
      ...
      if (val == 'SLOT' || val == 'LIVE') {
        ...
        return ajaxGetSessionPersonalData().then(res => {
          if (res.success) {
            // 更新钱包数据
            commit(types.UPDATE_MONEY, {
              'key': 'SLOT',
              'val': res.data.slotAccount
            })
            commit(types.UPDATE_MONEY, {
              'key': 'LIVE',
              'val': res.data.liveAccount
            })
    ...
    [types.UPDATE_USERDATA]({
      ...
      return ajaxGetSessionPersonalData().then(res => {
        if (res.success) {
          ...
          commit(types.UPDATE_MONEY, {
            key: 'SLOT',
            val: res.data.slotAccount || '0'
          })
          commit(types.UPDATE_MONEY, {
            key: 'LIVE',
            val: res.data.liveAccount || '0'
          })
          commit(types.UPDATE_MONEY, {
            key: 'MAIN',
            val: res.data.accountMoney || '0'
          })
          commit(types.UPDATE_MONEY, {
            key: 'DEPUTY',
            val: res.data.deputyCredit || '0'
          })
          return Promise.resolve(res)
        } else {
          return Promise.reject(res)
