<template>
  <div>
    <!-- 手机验证分两种，从接口读取，判断用哪种 -->
    <!-- 系统发送验证码给手机 -->
    <el-form ref="infoForm" label-width="100px" v-if="model=='0'&&showVerify">
    ...
    <div v-if="model=='1'&&showVerify">
      <el-alert
        :title="`发送短信${reverse.code}到${reverse.showGet}`"
        :description="`请使用${phone}手机号码，发送短信“${reverse.code}到${reverse.showGet}”。发生成功15秒后，点击“立即验证”按钮`"
        type="success"
        show-icon
      ></el-alert>
    ...
    <el-alert title="正在查询" description="正在发送..." type="primary" show-icon v-if="!showVerify"></el-alert>
  </div>
</template>
<script>
