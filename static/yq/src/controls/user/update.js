...
export const update = {
  ...
  methods: {
    checkComplete(obj) {
      ...
      if (obj.accountName && !check.name.test(obj.accountName)) {
        // 完善信息，姓名必填
        return '请输入正确格式中文姓名'
    ...
    modifySubmit(obj) {
      ...
      return new Promise((resolve, reject) => {
        ...
        modifyCustomerSocialInfo(o2bj).then(res => {
          if (res.success) {
  ...
  computed: {
    ...
    // 判断是否正在完善信息
    isCompleting() {
      if ((this.userData.accountName && this.userData.phone && this.userData.birthday) || this.isAgent) {
        return false
      } else {
        return true
      }
