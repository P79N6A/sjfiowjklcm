...
export const chessGames = {
  ...
  methods: {
    play(obj, e, device) {
      ...
      // }
      openWindow(url, '棋牌游戏', 900, 640, true, this.$store.getters.islogin);
    },
    /**
     * 获取游戏链接
    ...
    getGameUrl(item, isFun, device) {
      ...
      switch (item.category) {
        ...
        case 'DTQP':
          if (device == 'mobile') {
            url = `/game/gameDTQP.php?gameId=${item.code}&isfun=${isFunString}&deviceType=mobile&lobbyUrl=${localUrl}`
          } else {
            url = `/game/gameDTQP.php?gameId=${item.code}&isfun=${isFunString}&deviceType=pc&lobbyUrl=${localUrl}`
          }
          break
        default:
          break;
