    ...
    getPromos(type) {
      ...
      return new Promise((resolve, reject) => {
        queryLatestPreferential(this.searchData).then(res => {
          if (res.success) {
            this.allData = res.data.records.filter(item => {
              // 时间的转换，有待优化
              item.activityStartTime = item.activityStartTime.slice(0, 19)
              if (item.activityEndTime) {
                item.activityEndTime = item.activityEndTime.slice(0, 19)
              } else {
                item.activityEndTime = '待定'
              }
              return item
            })
