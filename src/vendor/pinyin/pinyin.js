...
const RE_PHONETIC_SYMBOL = new RegExp('([' + keys(PHONETIC_SYMBOL).join('') + '])', 'g');
...
//     single_pinyin:function(han, options) {
//       var self=this;
...
//         return [self.toFixed(pys[0], options.style)];
//       }
...
//         py = self.toFixed(pys[i], options.style);
...
class Pinyin {
  ...
  single_pinyin(han, options) {
    ...
    const pys = this._dict[hanCode].split(',');
    if (!options.heteronym) {
  ...
  static toFixed(pinyin, style) {
    ...
    switch (style) {
      ...
      case PINYIN_STYLE.NORMAL:
        return pinyin.replace(RE_PHONETIC_SYMBOL, ($0, $1_phonetic) => PHONETIC_SYMBOL[$1_phonetic].replace(RE_TONE2, '$1'));

      case PINYIN_STYLE.TO3NE:
        return pinyin.replace(RE_PHONETIC_SYMBOL, ($0, $1_phonetic) => PHONETIC_SYMBOL[$1_phonetic]);

      case PINYIN_STYLE.TONE2:
        py = pinyin.replace(RE_PHONETIC_SYMBOL, ($0, $1) => {
        // 声调数值。
          tone = PHONETIC_SYMBOL[$1].replace(RE_TONE2, '$2');
