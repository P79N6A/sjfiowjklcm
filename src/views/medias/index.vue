<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="mediaList" border fit highlight-current-row style="width: 100%;">
      <el-table-column :label="$t('table.id')" type="index" align="center" width="50">
      </el-table-column>
      <el-table-column label="缩略图" prop="name">
        <template slot-scope="scope">
          <a target="_blank" v-if="scope.row.type.includes('image')" :href="`${cdnurl}${scope.row.src}`" class="img-view" :style="`background-image:url(${cdnurl}${scope.row.src});`">
          </a>
          <a v-else class="file-view">
            {{scope.row.fileName.substr(scope.row.fileName.lastIndexOf('.') + 1)}}
          </a>
        </template>
      </el-table-column>
        ...
        <template slot-scope="scope">
          <el-button-group>
            ...
            <el-tooltip class="item" effect="dark" content="编辑该文件" placement="top-start">
              <el-button v-if="scope.row.status!='deleted'" size="mini" type="primary" icon="el-icon-edit" @click="handleUpdate(scope.row)">
              </el-button>
            </el-tooltip>
            <el-tooltip class="item" effect="dark" content="在线预览" placement="top-start">
              <el-button v-if="scope.row.status!='deleted'" size="mini" type="danger" icon="el-icon-view" @click="handleOpen(`${cdnurl}${scope.row.src}`)">
              </el-button>
            </el-tooltip>
            <el-tooltip class="item" effect="dark" content="复制外链地址" placement="top-start">
              <el-button v-if="scope.row.status!='deleted'" size="mini" type="danger" icon="el-icon-printer"
                v-clipboard:copy="`${cdnurl}${scope.row.src}`" v-clipboard:success="clipboardSuccess"></el-button>
            </el-tooltip>
    ...
    <!-- 列表翻页 -->
    <pagination v-show="listQuery.pages>0" :total="listQuery.totals" :page.sync="listQuery.currentPage" :limit.sync="listQuery.pageSize"
      @pagination="getMedias" :autoScroll="false" />

    <!-- 上传弹窗 -->
    <el-dialog :visible.sync="dialogVisible" width="400px">
      <el-upload :multiple="true" ref="upload" :file-list="fileList" :show-file-list="true" :on-remove="handleRemove" :on-success="handleSuccess"
        :before-upload="beforeUpload" action="/api/media" list-type="text"  drag :auto-upload="false">
    ...
    <el-dialog title="编辑文件信息" :visible.sync="dialogFormVisible" width="500px">
      <el-form ref="dataForm" :rules="rules" :model="mediaTemp" label-position="right" label-width="70px" style="width: 400px; margin-left:50px;">
  ...
  export default {
    ...
    methods: {
      ...
      getMedias() {
        ...
        getMedias(this.listQuery).then(res => {
      ...
      handleUpdate(row) {
        ...
        // str.lastIndexOf("/")
        // str.substr(）
        this.mediaTemp.name = row.fileName.substr(0, row.fileName.lastIndexOf('.'))
        this.mediaTemp.type = row.fileName.substr(row.fileName.lastIndexOf('.') + 1)
      ...
      updateFile() {
        ...
        updateMedias(postData).then(res => {
          this.getMedias()
          this.$notify({
            title: '成功',
            message: '操作成功',
      ...
      handleSubmit() {
        ...
        if (!this.checkAllSuccess()) {
          this.$message('请等待所有文件上传成功,或出现了网络问题，请刷新页面重新上传！')
          return
        }
