<template>
  <div class="itgo-container">
    ...
    <div class="contain">
      ...
      <div class="center">
        <!-- 传入当前页的json -->
        <Ishow :pageJson="appJson.value.pageJson[activePageIndex]" :activeTempIndex="activeTempIndex"></Ishow>
        <br>
      </div>
      <div class="right">
        <Itool :appJson="appJson" :activePageIndex="activePageIndex" :activeTempIndex="activeTempIndex"></Itool>
      </div>
    </div>
  </div>
</template>
    ...
    methods: {
      ...
      toggleTempLock(index) {
        this.appJson.value.pageJson[this.activePageIndex].json[
            index
          ].config.isLock = !this.appJson.value.pageJson[this.activePageIndex].json[index]
          .config.isLock;
      },
      // 切换元素显示与否
      toggleTempShow(index) {
        this.appJson.value.pageJson[this.activePageIndex].json[
            index
          ].config.isShow = !this.appJson.value.pageJson[this.activePageIndex].json[index]
          .config.isShow;
      },
      // 更新元素排序
