i<template>
  <div @click.stop>
    ...
    <div v-if="eleJson.type===1" class="" :style="[borderCss,baseCss,bgCss,textCss,boxShadow]">
      <div class="" v-html="eleJson.config.content">
      </div>
    </div>
    <!-- 图片 -->
    <div v-else-if="eleJson.type===2" class="" :style="[borderCss,baseCss,bgCss,textCss,boxShadow]">
      <img :src="eleJson.config.content" alt class="">
    </div>
    <!-- 视频 -->
    <div v-else-if="eleJson.type===2" class="" :style="[borderCss,baseCss,bgCss,textCss,boxShadow]">
      <video :src="eleJson.config.content" controls="controls">
        您的浏览器不支持 video 标签。
      </video>
    </div>
  </div>
...
<script>
  export default {
    ...
    computed: {
      ...
      boxShadow(){
        return {
          boxShadow:`${this.eleJson.style.shadow.shadowColor} ${this.eleJson.style.shadow.shadowX}px ${this.eleJson.style.shadow.shadowY}px ${this.eleJson.style.shadow.shadowFuzzy}px ${this.eleJson.style.shadow.shadowDire}px ${this.eleJson.style.shadow.shadowinSet?'inset':''}`
        }
      }
    }
  };
