...
<div :class="pageJson.animate.enterAnimation" class="animated">
  <div class="i-show" :style="[baseCss,borderCss,bgCss,boxShadow]">
    <!-- 拖拽外框 -->
    <VueDragResize :isActive="activeTempIndex==i" v-on:resizing="resize" v-on:dragging="resize" v-for="(drag,i) in pageJson.json"
      v-show="drag.config.isShow" :isDraggable="!drag.config.isLock" :isResizable="!drag.config.isLock" :parentW="1000" :parentH="1000" :w="drag.style.base.width" :minw="1" :minh="1"
      :h="drag.style.base.height" :x="drag.position.left" :y="drag.position.top" :z="1000-i" :key="i" @activated="$bus.$emit('selectTemp',i)">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      boxShadow(){
        return {
          boxShadow:`${this.pageJson.shadow.shadowColor} ${this.pageJson.shadow.shadowX}px ${this.pageJson.shadow.shadowY}px ${this.pageJson.shadow.shadowFuzzy}px ${this.pageJson.shadow.shadowDire}px ${this.pageJson.shadow.shadowinSet?'inset':''}`
        }
      }
    }
  };
