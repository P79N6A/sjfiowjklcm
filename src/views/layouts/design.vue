<template>
  <div class="LayoutDesign-container">
    <div class="i-header">
      <div class="content">
        <div id="clone">
          <el-button v-show="editting" type="danger" class="clone-row" @click="addRow(moveEndData.from.rowIndex)">添加区块</el-button>
          <el-button v-show="editting" type="warning" @click="addContent(moveEndData.from.rowIndex,moveEndData.from.contentIndex)">添加行</el-button>
          <el-button v-show="editting" type="primary" @click="addCol(moveEndData.from.rowIndex,moveEndData.from.contentIndex)">
            添加内容块
          </el-button>
    ...
    <!-- 页面操作区域 -->
    <div class="drag-box" id="drag" ref="imageWrapper" :style="getStyle(layoutTemp.value.style)" v-if="layoutTemp._id">
    ...
    <el-dialog title="设置" width="800px" :visible.sync="dialogFormVisible">
      <el-form ref="dataFormKey" :model="layoutTemp" label-position="right" label-width="80px">
        ...
        <el-form-item label="*主体宽度">
          <el-input type="textarea" v-model="layoutTemp.value.styleText" :autosize="{ minRows: 10, maxRows: 20}"
            placeholder="请粘贴入style样式内容"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
  ...
  export default {
    ...
    methods: {
      ...
      colMoveSet() {
        ...
        Sortable.create(byId("drag"), {
          ...
          onEnd: evt => {
            ...
            // 筛出数据
            const targetCol =
              $this.layoutTemp.value.rows.splice(
                evt.oldIndex,
                1
              )[0];
            // 插入数据
        ...
        [].forEach.call(byId("drag").getElementsByClassName("rows"), function (
          ...
          Sortable.create(block, {
            ...
            onEnd: evt => {
              ...
              // 筛出数据
              const targetCol =
                $this.layoutTemp.value.rows[$this.moveEndData.from.rowIndex].contents.splice(
                  $this.moveEndData.from.elIndex,
                  1
                )[0];
              // 插入数据
              $this.layoutTemp.value.rows[$this.moveEndData.to.rowIndex].contents.splice(
          ...
          [].forEach.call(block.getElementsByClassName("cols"), function (el, j) {
            Sortable.create(el, {
              ...
              onEnd: evt => {
                ...
                // 筛出数据
                const targetCol = $this.layoutTemp.value.rows[$this.moveEndData.from.rowIndex].contents[
                  $this.moveEndData.from.contentIndex].boxs.splice(
                  $this.moveEndData.from.elIndex,
                  1
                )[0];
                // 插入数据
                $this.layoutTemp.value.rows[$this.moveEndData.to.rowIndex].contents[$this.moveEndData.to.contentIndex]
                  .boxs.splice(
                    $this.moveEndData.to.elIndex,
                    0,
                    targetCol
      ...
      addContent(rowIndex, contentIndex) {
        ...
        this.layoutTemp.value.rows[rowIndex].contents.splice(contentIndex + 1, 0, newRow);
      ...
      addCol(rowIndex, contentIndex) {
        ...
        this.layoutTemp.value.rows[rowIndex].contents[contentIndex].boxs.push(newCol);
        this.$nextTick(() => {
          this.colMoveSet();
        });
      },
