<template>
  <div class="LayoutDesign-container">
    ...
    <div class="i-header" v-show="status!='preview'">
      <div class="content">
        <div id="clone">
          <el-button v-show="status=='layoutEdit'" type="danger" class="clone-row" @click="addRow(moveEndData.from.rowIndex)">添加区块</el-button>
          <el-button v-show="status=='layoutEdit'" type="warning" @click="addContent(moveEndData.from.rowIndex,moveEndData.from.contentIndex)">添加行</el-button>
          <el-button v-show="status=='layoutEdit'" type="primary" @click="addCol(moveEndData.from.rowIndex,moveEndData.from.contentIndex)">添加内容块</el-button>
    ...
    <!-- 预览工具栏 -->
    <el-radio-group v-model="status" v-show="status=='preview'" style="position:fixed;top:30px;left:50%;z-index:9999;transform:translateX(-50%);">
      <el-radio-button label="preview">预览模式</el-radio-button>
      <el-radio-button label="contentEdit">编辑内容</el-radio-button>
      <el-radio-button label="layoutEdit">编辑布局</el-radio-button>
    </el-radio-group>
    ...
    <div v-if="status=='preview'">
      ...
      <div class="drag-box" :class="status" id="drag" ref="imageWrapper" :style="getStyle(layoutTemp.value.style)" v-if="layoutTemp._id">
        <div class="rows" v-for="(row,i) in layoutTemp.value.rows" :key="i" :style="getStyle(row.style)">
          <div class="contents" v-for="
          (content,j) in row.contents" :key="j" :style="[getStyle(content.style),{width:content.fullWidth?'100%':layoutTemp.value.contentWidth+'px'}]">
            <el-row class="cols">
              <el-col class="box" v-for="(box,k) in content.boxs" :key="k" :span="box.width">
                <div class="view" :style="getStyle(box.style)">
                  <div v-for="(item,l) in box.components" :key="l" style="position:relative;" class="components-col">
    ...
    <div v-if="status=='contentEdit'">
      <div class="drag-box" :class="status" id="drag" ref="imageWrapper" :style="getStyle(layoutTemp.value.style)" v-if="layoutTemp._id">
        <div class="rows" v-for="(row,i) in layoutTemp.value.rows" :key="i" :class="{active:i==moveEndData.from.rowIndex}"
          ...
          <div class="contents" v-for="
          (content,j) in row.contents" :key="j" :class="{active:moveEndData.from.rowIndex==i&&j==moveEndData.from.contentIndex}"
