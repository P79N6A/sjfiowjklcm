<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="messageList" border fit highlight-current-row style="width: 100%;">
        ...
        <template slot-scope="scope">
          <el-button-group>
            ...
            <el-tooltip class="item" effect="dark" content="查看" placement="top-start">
              <el-button size="mini" type="danger" icon="el-icon-view" @click="handleView(scope.row)"></el-button>
            </el-tooltip>
          </el-button-group>
        </template>
      </el-table-column>
    ...
    <pagination v-show="listQuery.pages>0" :total="listQuery.totals" :page.sync="listQuery.currentPage" :limit.sync="listQuery.pageSize"
      @pagination="getMessages" :autoScroll="false" />

    <!-- 发送站内信弹窗 -->
    <el-dialog :title="textMap[dialogStatus]" :close-on-click-modal="false" :visible.sync="dialogFormVisible" width="800px"
      v-if="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="messageTemp" label-position="right" label-width="70px">
    ...
    <el-dialog  :title="messageTemp.title" :visible.sync="dialogViewVisible" width="800px" v-if="messageTemp.author&&messageTemp.author.nickname">
      <el-card class="box-card">
        <div slot="header" class="clearfix">
          ...
          <div>类型：<el-tag type="primary">{{messageTemp.type}}</el-tag></div>
        </div>
        <div v-html="messageTemp.content">
          站内信详情部分
        </div>
      </el-card>
    </el-dialog>
  ...
  export default {
    ...
    methods: {
      ...
      handleView(row){
        ...
        getMessageOne({_id:row._id}).then(res=>{
          this.$nextTick(() => {
      ...
      // 发送站内信
      addMessage() {
        addMessages(this.messageTemp).then(res => {
          this.getMessages()
      ...
      handleCheckAllChange(val) {
        this.messageTemp.recId = val ? this.recId.map(item => {
          return item._id
        }) : [];
      ...
      handleCheckedCitiesChange(value) {
        ...
        this.isIndeterminate = checkedCount > 0 && checkedCount < this.recId.length;
      }
    },
    computed: {
      // ...mapGetters(["roles"])
