<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" v-if="requireList.length>0" :data="requireList" border fit highlight-current-row
      style="width: 100%;">
      <el-table-column :label="$t('table.id')" type="index" align="center" width="50" fixed></el-table-column>
      <el-table-column :label="$t('table.id')" align="center" width="180" fixed prop="_id"></el-table-column>
      <el-table-column label="状态" align="center" width="70">
        <template slot-scope="scope">
          <el-tag type="success" v-if="scope.row.status=='processing'">{{getStatus(scope.row.status)}}</el-tag>
          <el-tag type="info" v-else-if="scope.row.status=='rejected'">{{getStatus(scope.row.status)}}</el-tag>
          <el-tag type="warning" v-else-if="scope.row.status=='completed'">{{getStatus(scope.row.status)}}</el-tag>
          <el-tag type="danger" v-else-if="scope.row.status=='inspection'||scope.row.status=='confirmed'">{{getStatus(scope.row.status)}}</el-tag>
      ...
      <el-table-column label="内容" align="center" width="50">
        <template slot-scope="scope">
          <el-tooltip class="item" effect="dark" content="点击查看内容" placement="top-start">
            <span @click="hadleView(scope.row)" style="text-align:center;cursor:pointer;margin:0 auto;display:inline-block;width:100%;">
      ...
      <el-table-column label="附件" align="center">
        <template slot-scope="scope">
          <div v-for="(file,i) in scope.row.media" :key="i">
            <a target="_blank" :href="cdnurl+file.src">
              {{file.name}}
            </a>
          </div>
        </template>
      ...
      <el-table-column label="预计时间" align="center" prop="preTime" width="90">
        <template slot-scope="scope" v-if="scope.row.preTime">{{scope.row.preTime| parseTime('{y}-{m}-{d} {h}:{i}') }}</template>
      </el-table-column>
      ...
      <el-table-column label="需求类型" align="center">
        <template slot-scope="scope">
          <el-tag type="success" v-if="scope.row.type=='om'||scope.row.type=='dev'">{{getType(scope.row.type)}}</el-tag>
    ...
    <!-- 列表翻页 -->
    <pagination :total="listQuery.totals" :page.sync="listQuery.currentPage" :limit.sync="listQuery.pageSize"
      @pagination="getRequirements" :autoScroll="false" />

    <!-- 发送/编辑需求弹窗 -->
    <el-dialog :title="textMap[dialogStatus]" :close-on-click-modal="false" :visible.sync="dialogFormVisible" width="800px"
      v-if="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="requireTemp" label-position="right" label-width="70px">

        <el-form-item label="紧急程度" prop="name">
          <el-radio-group v-model="requireTemp.emergency" :disabled="dialogStatus=='update'">
            <el-radio :label="item.value" v-for="item in emergencyType" :key="item.value">{{item.name}}</el-radio>
          </el-radio-group>
        </el-form-item>

        <el-form-item label="所属项目" prop="device">
          <el-select v-model="requireTemp.project" placeholder="请选择所属项目" :disabled="dialogStatus=='update'">
            <el-option :label="project.name" :value="project._id" v-for="(project,i) in projectsList" :key="i"></el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="附件" prop="title" v-if="dialogStatus=='create'">
          <el-upload class="upload-demo" action="/api/requirements/file" :data="requireTemp" ref="files" multiple
        ...
        <div v-if="dialogStatus=='update'">
          ...
          <el-form-item label="相关人员" prop="recId">
            <el-checkbox-group v-model="requireTemp.processor">
              <el-checkbox v-for="user in userList" :label="user._id" :key="user._id">{{user.nickname}}</el-checkbox>
            </el-checkbox-group>
          </el-form-item>
          <el-form-item label="预计时间" prop="preTime">
            <el-date-picker v-model="requireTemp.preTime" type="datetime" placeholder="选择日期时间">
            </el-date-picker>
          </el-form-item>

          <el-form-item label="需求状态" prop="status">
            <el-select v-model="requireTemp.status" placeholder="请选择所属项目">
              <el-option :label="statu.name" :value="statu.value" v-for="(statu,i) in statusList" :key="i"></el-option>
            </el-select>
          </el-form-item>
      ...
      进度信息
      <hr>
      <div v-for="(answer,i) in answerList" :key="i" style="background:#eee;border:solid 1px #ccc;padding:10px;margin-bottom:10px;position:relative;">
        <p>{{answer.author.nickname}} 回复:</p>
        <div v-html="answer.content"></div>
        <span class="el-icon-circle-close-outline" style="position:absolute;top:20px;right:20px;cursor:pointer;font-size:20px;"
          @click="deleteAnswers(answer)"></span>
      </div>
      <div>
    ...
    <el-dialog title="添加回复" :visible.sync="dialogAnswerVisible" width="800px" v-if="dialogAnswerVisible">
      <tinymce :height="400" v-model="answerTemp.content"></tinymce>
      <div slot="footer" class="dialog-footer">
        <el-button @click="addAnswer" type="primary">提交</el-button>
      </div>
    </el-dialog>
    <el-dialog :title="requireTemp.title" :visible.sync="dialogViewVisible" width="800px" v-if="dialogViewVisible">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleUpdate() {
        updateRequiresments(this.requireTemp).then(res => {
          this.$notify({
            title: "成功",
            message: "操作成功",
            type: "success",
      ...
      // 添加回复
      addAnswer() {
        addAnswers(this.answerTemp).then(res => {
