<template>
  <div class="imgs-container">
    <div>
      ...
      <span>
        <el-button type="primary" @click="dialogInsertVisible=true" icons="el-icon-upload">批量插入</el-button>
      </span>
    </div>
    <br>
    <el-row>
    ...
    <el-row>
      ...
      <el-col :span="22">
        <el-radio-group v-model="filterData.color" size="mini">
          <el-radio-button :label="type.value" v-for="type in colorList" :key="type.value">{{type.name}}</el-radio-button>
        </el-radio-group>
      </el-col>
    </el-row>
    <br>
    <el-table v-loading="listLoading" :data="imgList" border fit highlight-current-row style="width: 100%;">
      ...
      <el-table-column fixed align="center" label="封面" prop="thumbnail" width="80">
        <template slot-scope="scope">
          <a target="_blank" :href="`${cdnurl}${scope.row.url}`" class="img-view" :style="`background-image:url('${cdnurl}${scope.row.url}');`"></a>
        </template>
      </el-table-column>
      ...
      <el-table-column label="创建时间" prop="date" min-width="150px"></el-table-column>
    ...
    <el-dialog :title="`游戏数据--${textMap[dialogStatus]}`" :visible.sync="dialogFormVisible" width="900px"
              ...
              <tr>
                <td>
                  <el-form-item label="封面" prop="thumbnail">
                    <el-upload class="avatar-uploader" action="/api/imgs" :multiple="true" :show-file-list="true"
                      :on-success="handleAvatarSuccess" :before-upload="beforeAvatarUpload" :data="imgTemp"
                      :auto-upload="false" list-type="text" ref="gameIcon" :limit="1" :on-change="handleChange"
                      :on-remove="handleRemove" drag v-if="!imgTemp.url">
                      <i class="el-icon-plus avatar-uploader-icon"></i>
                    </el-upload>
                    <a v-if="imgTemp.url" class="img-view" style="width:120px;height:120px;display:block;" :style="`background-image:url('${cdnurl}${imgTemp.url}');`"></a>
                ...
                <td>
                  <el-form-item label="是否公开" prop="isPublic">
                    <el-switch v-model="imgTemp.isPublic" active-color="#13ce66" inactive-color="#ff4949" active-text="是"
                ...
                <td>
                  <el-form-item label="颜色" prop="color">
                    <el-radio-group v-model="imgTemp.color" size="mini">
                      <el-radio :label="type.value" v-for="type in colorList" :key="type.value">{{type.name}}</el-radio>
                    </el-radio-group>
                  </el-form-item>
                </td>
              </tr>
              <tr>
                <td>
                  <el-form-item label="描述" prop="desc">
                    <el-input type="textarea" :autosize="{ minRows: 4, maxRows: 8}" v-model="imgTemp.desc"></el-input>
                  </el-form-item>
                </td>
                <td>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="dialogStatus==='create'?createImg():updateImg()">{{ $t('table.confirm') }}</el-button>
      </div>
    </el-dialog>
    <el-dialog :visible.sync="dialogInsertVisible" :append-to-body="true">
      <el-upload :multiple="true" :show-file-list="true" class="editor-slide-upload" action="/api/imgs" :auto-upload="false"
        list-type="text" ref="gameIconInsert">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleSubmit() {
        this.$refs.gameIconInsert.submit();
