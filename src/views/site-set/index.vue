      ...
      <div class="text item">
        <el-form label-position="left" label-width="80px" :model="siteInfo.value">
          ...
          <el-form-item label="网站ico">
            <el-upload class="avatar-uploader" action="/api/media" :show-file-list="false" :on-success="handleAvatarSuccessFav"
              :before-upload="beforeAvatarUploadFav">
              <img v-if="siteInfo.value.favicon" :src="cdnurl+siteInfo.value.favicon" class="favicon">
              <i v-else class="el-icon-plus avatar-uploader-icon"></i>
            </el-upload>
          </el-form-item>

          <el-form-item label="网站logo">
            <el-upload class="avatar-uploader" action="/api/media" :show-file-list="false" :on-success="handleAvatarSuccessLogo"
      ...
      <div class="text item">
        <el-form label-position="left" label-width="80px" :model="siteInfo">
          <el-form-item label="代码(顶部)">
            <el-input v-model="siteInfo.value.codeHeader" type="textarea" :autosize="{ minRows: 5, maxRows: 10}"></el-input>
            <div>这里的代码将注入到网站每个页面头部</div>
          </el-form-item>
          <el-form-item label="代码(脚部)">
            <el-input v-model="siteInfo.value.codeFooter" type="textarea" :autosize="{ minRows: 5, maxRows: 10}"></el-input>
      ...
      <div class="text item">
        <el-form label-position="left" label-width="80px" :model="siteInfo">
          <el-form-item label="网站维护">
            <el-switch style="display: block" v-model="siteInfo.value.maintain" active-color="#13ce66" inactive-color="#ff4949"
...
<script>
  ...
  export default {
    ...
    methods: {
      saveSiteInfo() {
        siteInfoPut(this.siteInfo).then(res => {
          this.$notify({
            title: '成功',
            message: '操作成功',
            type: 'success',
