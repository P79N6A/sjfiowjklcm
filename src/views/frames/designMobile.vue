<template>
  <div class="FrameDesigh-container">
    ...
    <div class="drag-box" id="drag">
      <div
        class="rows"
        v-for="(row,i) in frameTemp.value.rows"
        :key="i"
      ...
      >
        ...
        <div
          class="contents"
          v-for="(content,j) in row.contents"
          :key="j"
...
<script>
import { getFramesOne, updateFrames, addFrames } from "@/api/frames";
...
export default {
  ...
  methods: {
    ...
    colMoveSet() {
      ...
      Sortable.create(byId("drag"), {
        ...
        onEnd: evt => {
          console.log(evt);
          // 筛出数据
          const targetCol = $this.frameTemp.value.rows.splice(
            evt.oldIndex,
            1
          )[0];
          // 插入数据
          $this.frameTemp.value.rows.splice(evt.newIndex, 0, targetCol);
          // 更改数据，强制渲染当前列表
      ...
      [].forEach.call(byId("drag").getElementsByClassName("rows"), function(
        ...
        Sortable.create(block, {
          ...
          onEnd: evt => {
            ...
            // 筛出数据
            const targetCol = $this.frameTemp.value.rows[
    ...
    addContent(rowIndex, contentIndex) {
      ...
      this.frameTemp.value.rows[rowIndex].contents.splice(
        contentIndex + 1,
        0,
        newRow
      );
    ...
    addCol(rowIndex, contentIndex) {
      ...
      this.frameTemp.value.rows[rowIndex].contents[contentIndex].boxs.push(
        newCol
      );
      this.$nextTick(() => {
        this.colMoveSet();
