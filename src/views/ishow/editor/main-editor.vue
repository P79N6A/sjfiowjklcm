    ...
    <div class="ishow-editorWrapper" @click.stop="">
      <el-tabs v-model="activeName2" type="border-card" v-if="!showBg&&showId" v-el-drag-dialog>
        ...
        <el-tab-pane label="图层" name="third">
          <div v-for="(item,i) in pageJson.json" :key="i" class="edit-layer" @click="layerSelect(item.id)">
            <div v-if="item.type==1">
              <i class="el-icon-tickets"></i>[{{item.zIndex}}]{{item.content}}
            </div>
            <div v-if="item.type==2">
              <i class="el-icon-picture-outline"></i>[{{item.zIndex}}]图片
            </div>
            <div v-if="item.type==3">
              <i class="el-icon-edit-outline"></i>[{{item.zIndex}}]输入框
            </div>
            <div v-if="item.type==4">
              <i class="el-icon-more-outline"></i>[{{item.zIndex}}]单选
            </div>
            <div v-if="item.type==5">
              <i class="el-icon-more"></i>[{{item.zIndex}}]多选
            </div>
            <div v-if="item.type==6">
              <i class="el-icon-d-caret"></i>[{{item.zIndex}}]下拉菜单
            </div>
            <div v-if="item.type==7">
              <i class="el-icon-success"></i>[{{item.zIndex}}]按钮
            </div>
            <!-- {{item}} -->
          </div>
        </el-tab-pane>
