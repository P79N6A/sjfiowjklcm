        ...
        <div class="layer-main_pic">
            <div class="layer-main_content">
                ...
                <div class="layer-main_box">
                  <!-- {{picJson}} -->
                    <div v-if="!isLoading">
                        <div v-for="(item,index) in picJson" class="layer-main_item" :key="index" :class="{ 'selected': isActive[index]}" :style="`background-image: url(${cdnurl}/${item.src})`" :data-url="item"  @mouseover="picHover(index,true)" @mouseout="picHover(index,false)">
                            <!-- {{item.src}} -->
                            <div v-show="isHover[index]" class="layer-main_item--pic">
                                <div class="tc">
                                    <el-button type="primary" @click="addElement(cdnurl+'/'+item.src,index)" class="mt20">
                                        <span v-if="isActive[index]">取消</span>
                                        <span v-else>选择</span>
                                    </el-button>
                                </div>
                                <div class="tc">
                                    <el-button @click="preview(cdnurl+'/'+item.src,index)" class="mt5" size="small"><i class="fa fa-eye" aria-hidden="true"></i> 预览</el-button>
                                </div>
                            </div>
                        </div>
                    </div>
