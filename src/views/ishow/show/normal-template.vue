  ...
  >
    ...
    >
      <div class="element-contents text-item">
        <div
          class="ishow-elementItem ishow-elementItem_text ishow-elementItem_drap"
          @click.stop="selectElement()"
          @contextmenu.prevent="rightClick($event)"
          v-html="content"
          v-show="!typewriter.isActive"
          :style="{
                  minHeight: resizeH
                }"
        ></div>
        <div
          class="ishow-elementItem ishow-elementItem_text"
          @click.stop="selectElement()"
          @contextmenu.prevent="rightClick($event)"
          v-html="typewriter.content"
          v-show="typewriter.isActive"
        ></div>
      </div>
    </div>
    ...
    >
      <div class="element-contents text-item">
        <div
          class="ishow-elementItem ishow-elementItem_drap"
          @click.stop="selectElement()"
          @contextmenu.prevent="rightClick($event)"
        >
          <textarea
    ...
    >
      <div class="element-contents text-item">
        <div
          class="ishow-elementItem ishow-elementItem_drap ishow-elementItem_radio tl"
          @click.stop="selectElement()"
          @contextmenu.prevent="rightClick($event)"
        >
          <div
    ...
    >
      <div class="element-contents text-item">
        <div
          class="ishow-elementItem ishow-elementItem_drap ishow-elementItem_checkbox tl"
          @click.stop="selectElement()"
          @contextmenu.prevent="rightClick($event)"
        >
          <div
          ...
          <el-checkbox-group v-model="checkbox1" :class="{vertical:json.form.dire==='v'}">
            <el-checkbox
              v-for="radioItem2 in json.form.options"
              :label="radioItem2.id"
              :key="radioItem2.id"
            >{{radioItem2}}</el-checkbox>
          </el-checkbox-group>
        </div>
      </div>
    </div>
    ...
    >
      <div class="element-contents text-item">
        <div
          class="ishow-elementItem ishow-elementItem_drap ishow-elementItem_select tl"
          @click.stop="selectElement()"
          @contextmenu.prevent="rightClick($event)"
        >
          <el-select
    ...
    >
      <div class="element-contents text-item">
        <div
          class="ishow-elementItem ishow-elementItem_drap ishow-elementItem_button tc"
          @click.stop="selectElement()"
          @contextmenu.prevent="rightClick($event)"
        >
          <a
  ...
  methods: {
    ...
    changeWH() {
      ...
      if (this.elHeight === "auto") {
        ...
        this.elHeight =
          this.type === 2 ? this.elWidth / this.imgSize.r : rect.height;
        this.triggerApp();
      }
    },
    // 修正样式值
    ...
    draggable(this.$el, {
      ...
      stop: (event, element) => {
        ...
        if (
          ...
          event.target.className.indexOf("ishow-elementItem_drap") === -1
        ) {
          return false;
        }
        if (event.button === 2) {
